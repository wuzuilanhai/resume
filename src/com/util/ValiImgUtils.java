...
/**
 * 类描述：验证码生成工具类（servlet实现）
 * 
 * @author: 张海彪
 * @date： 日期：2016-8-3 时间：下午4:56:57
 * @version 1.0
 */
@SuppressWarnings("serial")
public class ValiImgUtils extends HttpServlet {
	...
	public void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		for (int i = 0; i < 5; i++) {
			g.drawLine(getRandInt(0, width), getRandInt(0, height),
					getRandInt(0, width), getRandInt(0, height));
		}
		...
		for (int i = 0; i < 4; i++) {
			...
			String cs = base.charAt(getRandInt(0, base.length() - 1)) + "";
			buffer.append(cs);
			g.drawString(cs + "", 5 + width / 4 * i, 22);
			g.rotate(-c, 5 + width / 4 * i, 22);
		}
		// 保存验证码串到session中
